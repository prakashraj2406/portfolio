...
function Toast({ type, message, onClose }) {
  ...
  return (
    ...
    `}>
      ...
      <span>{message}</span>
      <button onClick={onClose} className="ml-2 opacity-70 hover:opacity-100 transition-opacity cursor-pointer">
        <FaTimes />
      </button>
    </div>
  );
...
/* ─── Field error text ─── */
const Err = ({ msg }) => msg
  ? <span className="text-red-500 text-[11px] font-poppins mt-1 pl-1">{msg}</span>
  : null;
...
export default function Contact() {
  ...
  const [fields, setFields] = useState({ from_name: "", from_mail: "", subject: "", message: "" });
  ...
  const validate = () => {
    ...
    if (!fields.from_mail.trim())
      e.from_mail = "Email is required.";
  ...
  const handleSubmit = (e) => {
    ...
    setSending(true);
    emailjs.sendForm(EJS_SERVICE, EJS_TEMPLATE, form.current, { publicKey: EJS_KEY })
      .then(() => {
